"""
Intel Tiber AI Cloud — Sequential Deepfake Training Pipeline.

This trainer is designed for Intel Tiber AI Cloud's free development tiers
where you get massive VRAM (Intel Max GPU / Gaudi, 32-96GB) but limited
local disk storage.
...
PIPELINE (per dataset):
    ...
    5. DELETE the dataset files to free disk space
    6. Move to the next dataset — repeat

The model weights CARRY FORWARD from dataset to dataset, so by the end
the final checkpoint has learned from ALL datasets sequentially.

DEVICE SUPPORT:
    - Intel XPU (Arc / Max GPU):  torch.xpu  + bfloat16 autocast
    - Intel Gaudi (Habana):       Habana PyTorch bridge (auto-detected)
    - NVIDIA CUDA:                torch.cuda + fp16 AMP
    - CPU fallback:               bfloat16 autocast
...
USAGE (inside tmux on Intel Tiber):
    python -m backend.training.train_intel_tiber \\
        --data-root /home/intel/data \\
        --output-dir /home/intel/checkpoints \\
        --hf-repo your-username/deepfake-detector \\
        --epochs-per-dataset 1 --amp --resume

Then close your browser / shut down your PC. Training keeps running
inside tmux on Intel's servers.
"""
...
# ---------------------------------------------------------------------------
# All datasets — Kaggle slugs + label config
# ---------------------------------------------------------------------------

# (slug, name, folder_labels, default_label, media_type)
# media_type: "vision" or "audio"
DATASET_PIPELINE = [
    # === VISION ===
    {
...
def detect_device() -> torch.device:
    """Detect the best available accelerator: Intel XPU, Gaudi, CUDA, or CPU."""
    # Intel Gaudi (Habana) — detected via the Habana PyTorch bridge
    try:
        import habana_frameworks.torch.core as htorch  # noqa: F401
        if htorch.hpu.is_available():
            print("  [Device] Intel Gaudi (HPU) detected")
            return torch.device("hpu")
    except (ImportError, AttributeError):
        pass

    # Intel XPU (Arc / Max GPU)
    ...
    if torch.cuda.is_available():
        print(f"  [Device] NVIDIA CUDA detected: {torch.cuda.get_device_name(0)}")
        return torch.device("cuda")

    print("  [Device] No GPU detected — using CPU (will be slow)")
    return torch.device("cpu")


def get_device_memory_gb(device: torch.device) -> float | None:
    """Return total device memory in GB, if available."""
    if device.type == "cuda":
...
def index_dataset(dataset_dir: str, ds_config: dict, max_samples: int | None = None
...
def train_on_dataset(model, samples, criterion, optimizer, scaler, device,
                     ...
                     checkpoint_path) -> int:
    """
    Train on one dataset's samples. Returns the number of samples trained on.
    The model checkpoint is saved after training (carrying forward ALL prior learning).
    """
    if not samples:
    ...
    print(f"\n{'='*70}")
    print(f"  DATASET {dataset_idx + 1}/{len(DATASET_PIPELINE)}: {ds_config['name']}")
    ...
    # Save checkpoint after completing this dataset — carries ALL learning forward
...
def push_to_huggingface(checkpoint_path: str, repo_id: str, token: str | None = None):
    ...
    if not token:
        ...
        print("  Set it via:  export HF_TOKEN=hf_your_token_here")
    ...
    try:
        ...
        print(f"  [OK] Uploaded {checkpoint_name} to {repo_id}")
    except Exception as e:
    ...
    print(f"\n  Download your trained model anytime from:")
...
def run_pipeline(args: argparse.Namespace):
    print(f"\n{'='*70}")
    print(f"  INTEL TIBER AI CLOUD — SEQUENTIAL DEEPFAKE TRAINING")
    print(f"{'='*70}")
    ...
    print(f"  HF repo: {args.hf_repo or '(not set — skip push)'}")
    print(f"  Epochs per dataset: {args.epochs_per_dataset}")
    print(f"  Batch size: {args.batch_size}")
    print(f"  Datasets in pipeline: {len(DATASET_PIPELINE)}")
    print(f"{'='*70}\n")
    ...
    for i, ds in enumerate(pipeline):
        print(f"    {i+1}. [{ds['media_type'].upper()}] {ds['name']} ({ds['slug']})")
    print()

    # Build model
    # Determine modality from the first dataset in pipeline
    is_audio_pipeline = pipeline[0]["media_type"] == "audio"
    ...
    # DataParallel for multi-GPU (works on CUDA and XPU)
    ...
    args.checkpoint_path = seq_ckpt  # used by save_checkpoint internally
    ...
    # Filter pipeline to only remaining datasets
    remaining = pipeline[start_idx:] if start_idx < len(pipeline) else []

    if not remaining:
        print("\n  All datasets already trained! Pushing to Hugging Face...")
        if args.hf_repo:
            push_to_huggingface(best_ckpt if os.path.exists(best_ckpt) else seq_ckpt,
                                args.hf_repo, args.hf_token)
        return

    print(f"\n  Starting at dataset index {start_idx}, {len(remaining)} datasets remaining\n")
    ...
    for i, ds_config in enumerate(remaining):
        ...
        print(f"\n  [Step 1] Downloading dataset: {ds_config['slug']}")
        ok = download_kaggle_dataset(ds_config["slug"], ds_dir)
        if not ok:
            print(f"  [SKIP] Could not download {ds_config['slug']} — moving to next")
            continue
        ...
        # 3. Train on it (checkpoint carries forward)
        ...
        # Also save to the "best" path (sequential: latest = best)
        shutil.copy2(seq_ckpt, best_ckpt)

        # 4. Delete the dataset to free disk space
    ...
    if args.hf_repo:
        ...
        print(f"\n  No --hf-repo specified. Final checkpoint is at:")
        print(f"  {best_ckpt}")
        print(f"  To push to HF later:")
        print(f"  python -c \"from huggingface_hub import HfApi; "
              f"HfApi().upload_file('{best_ckpt}', '{checkpoint_name}', "
              f"'your-repo/id', token='hf_xxx')\"")


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Intel Tiber AI Cloud — Sequential Deepfake Training Pipeline"
    )
    parser.add_argument("--data-root", default="/home/intel/data",
                        help="Where to download datasets (will be cleaned up after each)")
    parser.add_argument("--output-dir", default="/home/intel/checkpoints",
                        help="Where to save checkpoints (persistent)")
    ...
    parser.add_argument("--hf-repo", default=None,
                        help="HF repo to push final checkpoint (e.g. your-username/deepfake-detector)")
...
if __name__ == "__main__":
    ...
    # If modality is "both", run vision first then audio
